...
class TestMaxGroupActive(RestrictionTestCase):
    ...
    def test_fail_excess_all(self):
        ...
        holder1 = Mock(state=State.active, item=item, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder1)
        holder2 = Mock(state=State.active, item=item, _domain=Domain.ship, spec_set=Module(1))
    ...
    def test_mix_excess_one(self):
        ...
        holder1 = Mock(state=State.active, item=item1, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder1)
        item2 = self.ch.type_(type_id=2, group=92, attributes={Attribute.max_group_active: 2})
        holder2 = Mock(state=State.active, item=item2, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder2)
        restriction_error1 = self.get_restriction_error(holder1, Restriction.max_group_active)
        self.assertIsNotNone(restriction_error1)
        self.assertEqual(restriction_error1.max_group, 1)
    ...
    def test_pass_holder_non_ship(self):
        ...
        holder1 = Mock(state=State.active, item=item, _domain=None, spec_set=Module(1))
        self.track_holder(holder1)
        holder2 = Mock(state=State.active, item=item, _domain=None, spec_set=Module(1))
        self.track_holder(holder2)
        restriction_error1 = self.get_restriction_error(holder1, Restriction.max_group_active)
        self.assertIsNone(restriction_error1)
        restriction_error2 = self.get_restriction_error(holder2, Restriction.max_group_active)
