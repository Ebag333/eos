...
class TestImplantIndex(RestrictionTestCase):
    ...
    def test_fail(self):
        ...
        holder1 = Mock(state=State.offline, item=item, _domain=Domain.character, spec_set=Implant(1))
    ...
    def test_pass_different(self):
        ...
        holder1 = Mock(state=State.offline, item=item1, _domain=Domain.character, spec_set=Implant(1))
        holder2 = Mock(state=State.offline, item=item2, _domain=Domain.character, spec_set=Implant(1))
        self.track_holder(holder1)
        self.track_holder(holder2)
        restriction_error1 = self.get_restriction_error(holder1, Restriction.implant_index)
