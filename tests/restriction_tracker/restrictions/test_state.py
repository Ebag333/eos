...
class TestState(RestrictionTestCase):
    ...
    def test_state_lower(self):
        ...
        holder = Mock(state=State.online, item=item, _domain=Domain.character, spec_set=Module(1))
    ...
    def test_state_equal(self):
        item = self.ch.type_(type_id=1)
        item.max_state = State.active
        holder = Mock(state=State.active, item=item, _domain=Domain.character, spec_set=Module(1))
        self.track_holder(holder)
        restriction_error = self.get_restriction_error(holder, Restriction.state)
        self.assertIsNone(restriction_error)
        self.untrack_holder(holder)
    ...
    def test_state_higher(self):
        item = self.ch.type_(type_id=1)
        item.max_state = State.active
        holder = Mock(state=State.overload, item=item, _domain=Domain.character, spec_set=Module(1))
        self.track_holder(holder)
        restriction_error = self.get_restriction_error(holder, Restriction.state)
