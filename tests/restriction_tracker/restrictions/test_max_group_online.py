...
class TestMaxGroupOnline(RestrictionTestCase):
    ...
    def test_fail_excess_all(self):
        ...
        item = self.ch.type_(type_id=1, group=6, attributes={Attribute.max_group_online: 1})
    ...
    def test_mix_excess_one(self):
        ...
        item1 = self.ch.type_(type_id=1, group=92, attributes={Attribute.max_group_online: 1})
        holder1 = Mock(state=State.online, item=item1, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder1)
        item2 = self.ch.type_(type_id=2, group=92, attributes={Attribute.max_group_online: 2})
        holder2 = Mock(state=State.online, item=item2, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder2)
        restriction_error1 = self.get_restriction_error(holder1, Restriction.max_group_online)
        self.assertIsNotNone(restriction_error1)
        self.assertEqual(restriction_error1.max_group, 1)
    ...
    def test_mix_excess_original(self):
        ...
        item1 = self.ch.type_(type_id=1, group=61, attributes={Attribute.max_group_online: 1})
        holder1 = Mock(state=State.online, item=item1, _domain=Domain.ship, spec_set=Module(1))
        holder1.attributes = {Attribute.max_group_online: 2}
        self.track_holder(holder1)
        item2 = self.ch.type_(type_id=2, group=61, attributes={Attribute.max_group_online: 2})
        holder2 = Mock(state=State.online, item=item2, _domain=Domain.ship, spec_set=Module(1))
    ...
    def test_pass(self):
        ...
        item = self.ch.type_(type_id=1, group=860, attributes={Attribute.max_group_online: 2})
        holder1 = Mock(state=State.online, item=item, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder1)
        holder2 = Mock(state=State.online, item=item, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder2)
    ...
    def test_pass_holder_none_group(self):
        ...
        item = self.ch.type_(type_id=1, group=None, attributes={Attribute.max_group_online: 1})
        holder1 = Mock(state=State.online, item=item, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder1)
        holder2 = Mock(state=State.online, item=item, _domain=Domain.ship, spec_set=Module(1))
        self.track_holder(holder2)
    ...
    def test_pass_state(self):
        ...
        item = self.ch.type_(type_id=1, group=886, attributes={Attribute.max_group_online: 1})
    ...
    def test_pass_holder_non_ship(self):
        ...
        item = self.ch.type_(type_id=1, group=12, attributes={Attribute.max_group_online: 1})
        holder1 = Mock(state=State.online, item=item, _domain=None, spec_set=Module(1))
        self.track_holder(holder1)
        holder2 = Mock(state=State.online, item=item, _domain=None, spec_set=Module(1))
        self.track_holder(holder2)
        restriction_error1 = self.get_restriction_error(holder1, Restriction.max_group_online)
        self.assertIsNone(restriction_error1)
        restriction_error2 = self.get_restriction_error(holder2, Restriction.max_group_online)
